...
void motion_plan::search_straight(float len_target,float acc,float max_sp,float end_sp)
{
	...
	move_task->turn_gain_set.set_sp_gain(12.0, 0.05, 0.0);
...
void motion_plan::long_turn(const t_param *turn_param,t_run_pattern run_pt,const t_pid_gain *sp_gain,const t_pid_gain *om_gain)
{
	...
	if(ABS(turn_param->param->degree) == 90.0f)
	{
		t_bool r_fix,l_fix;
		r_fix = l_fix = False;
		if(SensingTask::getInstance().sen_r.is_wall == True && ABS(45.0 - SensingTask::getInstance().sen_r.distance) <= 15.0)
		{
			r_fix = True;
		}
		if(SensingTask::getInstance().sen_l.is_wall == True && ABS(45.0 - SensingTask::getInstance().sen_l.distance) <= 15.0)
		{
			l_fix = True;
		}
		float diff = move_task->mouse.x_point;
	...
	{
		float diff = 0.0f;//move_task->mouse.x_point;
		float fix_th = 15.0;
		...
		if(turn_param->param->turn_dir == Turn_L)
		{
			...
			{
				move_task->rT.turn_rmin_fix = 0.0f;//(-1.0)*PI/accel_Integral/10.0f*diff;
				mt_set_.rad_max_velo    =  turn_param->param->velo/(turn_param->param->r_min+move_task->rT.turn_rmin_fix)*1000.0f;
			}
		}
		else if(turn_param->param->turn_dir == Turn_R)
...
void motion_plan::fix_wall(float set_time)
{
	...
	move_task->rT.is_wallControl_Enable = Non_controll;
	SensingTask::getInstance().Division_Wall_Correction_Reset();
	move_task->straight_gain_set.set_sp_gain(12.0, 0.01, 0.0);
